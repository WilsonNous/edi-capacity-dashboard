...
def multiselect_filter(frame: pd.DataFrame, label: str, col: str):
...
if missing:
    st.warning("Algumas análises ficarão limitadas porque faltam estas colunas: " + ", ".join(missing))

st.sidebar.markdown("### Filtros")
assignees = multiselect_filter(df, "Atribuído a", "Atribuído a")
statuses = multiselect_filter(df, "Estado", "Estado")
priorities = multiselect_filter(df, "Prioridade", "Prioridade")
types = multiselect_filter(df, "Tipo", "Tipo")
projects = multiselect_filter(df, "Projeto", "Projeto")
clients = multiselect_filter(df, "Cliente", "Clientes")

f = df.copy()
for col, vals in [
    ("Atribuído a", assignees), ("Estado", statuses), ("Prioridade", priorities),
    ("Tipo", types), ("Projeto", projects), ("Clientes", clients)
]:
    if vals and col in f.columns:
        f = f[f[col].astype(str).isin(vals)]

st.sidebar.markdown("### Tempo em aberto")
aging_min = int(f["Tempo em aberto (dias)"].min()) if len(f) else 0
aging_max = int(f["Tempo em aberto (dias)"].max()) if len(f) else 0
aging_range = st.sidebar.slider("Faixa de dias em aberto", 0, max(aging_max, 1), (0, max(aging_max, 1)))
